  ...
  methods: {
    onSubmit (evt) {
      ...
      if (this.$root.$token !== '' && this.$root.$isValidToken) {
        ...
        this.$http.put('http://sigoapp.southcentralus.azurecontainer.io:3000/sigo/gestaoprocind', JSON.stringify(this.form), {headers})
          .then(res => {
          ...
          .catch(res => {
            if (res.status === 404 || res.status === 401) {
              ...
              alert('Produto não atualizado, verifique as informações')
            }
          })
      }
    },
